...
|---|---|---|---|---|---|---|
| AF-01 | A paper or index card in the inbox is really a sign pointing to a private file somewhere else. | A symlink, junction, special file, escaped configured directory, or same-stem JSON/Markdown companion is opened, parsed, hashed, transmitted, moved, or written before its own confinement and link checks. A confined PDF can therefore carry an absolute companion symlink outside the workspace; reading or updating that sidecar follows the link, and approval can move the link and later overwrite its external target. With a LAN or internet Ollama URL, OCR from an arbitrary local PDF can also leave the host. | T/I/E | Critical | Validate configured directories at load; require regular files; independently resolve and confine the PDF and every companion to the exact lifecycle root before any read, hash, inference request, move, or write; reject symlinks, junctions, hard-link surprises, and special files; recheck opened identities where supported. | Symlink, junction, hard-link, absolute-path, `..`, special-file, and platform-specific tests for PDF/JSON/Markdown proving the parser, hasher, inference client, movers, and writers are never called for an unconfined or linked artifact. |
...
| AF-04 | The suggested label secretly contains directions to another drawer—or makes the letter and index card share one name. | LLM output or a tampered sidecar supplies absolute paths, separators, traversal, reserved names, wrong extensions, or wrong lifecycle roots. A destination such as `report.json` makes the derived PDF and JSON destinations identical; the sidecar move can then replace the just-moved PDF even inside a valid root. Both scan and reprocess dry-run write immediately using the model-supplied filename, so either can escape `_DryRun`; reprocess reaches this path from a document already in `_Review`. | T/I/E | Critical | Ignore model path strings for writes; derive names locally from bounded semantic fields; enforce the canonical `.pdf` extension; derive companions from trusted code; require PDF/JSON/Markdown paths to be pairwise distinct under platform case/Unicode rules and filesystem identity; validate exact roots before every operation; validate sidecars on every use; no overwrite. | Cross-platform malicious-name and extension corpus for every scan/dry-run/approve/reject/reprocess destination, including `.json`, `.md`, empty/dot names, case aliases, Unicode aliases, reserved names, and paths resolving to the same object; prove no mover/writer runs before all pairwise-distinct and exact-root checks pass. |
...
| AF-06 | The letter moves but its index card does not, or the card is half-written when the power fails. | Sidecar writes can truncate authoritative state; during an ordinary non-dry-run scan, pre-move JSON and optional Markdown writes silently replace existing same-stem sibling files before collision handling, then the move removes the replacements from the inbox. Sequential PDF/JSON/Markdown moves can leave partial or contradictory state after interruption. In addition, explicit rejection and scan error handling always pass `move_markdown=False`: when Markdown output exists, PDF/JSON move to `_Error` while the sensitive `.md` is deterministically orphaned in the previous lifecycle area. | T/I/R/D | High | Collision-safe, no-overwrite sibling creation before normal scanning; transition the complete PDF/JSON/Markdown unit or explicitly journal every retained artifact; atomic temp-write, fsync where appropriate, replace only for an explicitly validated revision; operation journal or recoverable state machine; idempotent retry; startup reconciliation; collision-safe rollback. | Pre-existing sibling JSON/Markdown tests proving their bytes remain unchanged; rejection and scan-error tests with Markdown enabled proving no orphan remains; fault injection after every write/move step; restart/retry/reconciliation tests on supported filesystems. |
...
| AF-18 | The clerk turns an unreadable amount into “zero”—or reads the written word “false” as “yes.” | Permissive model/schema coercion silently converts malformed amounts to `0.0`, clamps out-of-range confidence percentages, maps unknown types to `Other`, and drops malformed collection entries. Configuration also uses loose Python coercion: a quoted non-empty value such as `"false"` becomes true and can unexpectedly enable Markdown, metadata rewriting, SQLite, or dry-run behavior. These conversions can hide invalid input and persist false data or activate consequential behavior. | T/R/E | High | Use strict typed schemas for model, sidecar, and configuration boundaries; reject wrong scalar/collection types and ambiguous consequential values; distinguish missing, invalid, model-proposed, and human-confirmed states; record bounded validation codes; require evidence and human review for amounts, deadlines, identities, and actions. | Adversarial locale/number corpus, quoted boolean and wrong-type configuration matrix, out-of-range scores, malformed collections, unknown enums, round-trip provenance, and CLI tests proving invalid configuration fails before filesystem, database, parser, inference, or metadata access. |
...
| AF-20 | The review desk and archive cabinet are configured as the same place. | Lifecycle directories can currently be absolute, equal, nested, case/Unicode aliases, or symlinks to the same filesystem object. If `_Review` aliases or sits inside `Archive`, a scan can place a document in the permanent archive before approval; other overlaps can bypass state boundaries or make cleanup affect another lifecycle. | T/R/E | Critical | Derive lifecycle roots from validated relative names; resolve with platform-aware normalization; require inbox, review, imported, error, split, dry-run, archive, and dedicated index locations to be pairwise distinct, non-overlapping, non-symlink directories beneath `base_dir`, and distinct from `base_dir` itself; fail configuration loading before creating anything. | Equal, parent/child, absolute, `..`, symlink/junction, case-fold, Unicode-normalization, short-name, and filesystem-identity alias configurations proving startup fails before directory, database, parser, inference, mover, or writer access. |
...
- non-empty LLM path suggestions can still influence destinations; a normal
  scan or reprocess can escape `_Review` (including directly into `Archive`)
  while remaining inside `base_dir`, and scan/reprocess dry-run use the suggested
  filename for an immediate, unconfined write that can escape `_DryRun` and
...
- companion JSON/Markdown paths are not independently checked for confinement,
  links, type, or filesystem identity, and a model filename with the wrong
...
- scan and reprocess dry-run both perform duplicate-index access when indexing
  is enabled (scan also initializes or updates the schema); both create
  `_DryRun`, persist model-named sidecars, and can escape that root or overwrite
  another JSON file, violating the mutation-free target invariant;
- with Markdown output enabled, rejection and scan error handling move PDF/JSON
  but pass `move_markdown=False`, deterministically leaving sensitive Markdown
  in the prior lifecycle area;
...
- pairwise-distinct, non-overlapping lifecycle and index roots validated before
  any directory, database, parser, inference, move, or write operation;
- canonical artifact extensions plus pairwise-distinct, independently confined,
  non-link PDF/JSON/Markdown paths for every lifecycle operation;
...
- separate scan, reprocess, approve, and reject dry-run tests proving that each
